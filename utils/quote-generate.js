...
export default class QuoteGenerate {
    ...
    async drawMultilineText(text, entities, fontSize, fontColor, textX, textY, maxWidth, maxHeight, emojiBrand = 'apple') {
        ...
        text = text.replace(/і/g, 'i'); // replacing the Ukrainian letter і with the English one, since it is missing in Noto fonts
        ...
        let lineX = textX, lineY = textY, textWidth = 0;
        ...
        for (let index = 0; index < styledWords.length; index++) {
            ...
            let fontType = '', fontName = 'NotoSans', fillStyle = fontColor;
    ...
    async drawAvatar(user) {
        ...
        if (avatarImage) {
            ...
            const avatarX = 0; const avatarY = 0;
    ...
    async drawQuote(scale = 1, backgroundColorOne, backgroundColorTwo, avatar, replyName, replyNameColor, replyText, name, text, media, mediaType, maxMediaSize) {
        ...
        let replyPosX = 0, replyNamePosY = 0, replyTextPosY = 0;
        if (replyName) {
        ...
        let mediaPosX = 0, mediaPosY = 0, mediaWidth, mediaHeight;
        ...
        const rectWidth = width - blockPosX; let rectHeight = height;
        const rectPosX = blockPosX; const rectPosY = blockPosY; const rectRoundRadius = 25 * scale;
    ...
    async generate(backgroundColorOne, backgroundColorTwo, message, width = 512, height = 512, scale = 2, emojiBrand = 'apple') {
        ...
        // user name  color;
        let nameIndex = 1;
        if (message && typeof message === 'object' && 'from' in message && message.from && typeof message.from === 'object' && 'id' in message.from && message.from.id) nameIndex = Math.abs(message.from.id) % 7;
        ...
        let nameCanvas;
        if (message && typeof message === 'object' && 'from' in message && message.from && typeof message.from === 'object' && 'name' in message.from && message.from.name) {
